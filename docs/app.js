...
async function loadSimulationData(scenario = "mumbai_tunnel") {
  ...
  // 1. Try local or deployed FastAPI server first
  try {
  ...
  // 2. Try static pre-cached JSON files (for GitHub Pages static deployment)
  try {
    const staticRes = await fetch(`./data/scenarios/${scenario}.json`);
    if (staticRes.ok) {
  ...
  // 3. Client-side procedural simulation fallback
  console.warn("Backend API unavailable, using fallback drive data");
...
async function fetchBenchmarks(scenario = "mumbai_tunnel") {
  let data = null;
  // 1. Try local or deployed backend
  try {
    const res = await fetch(`/api/benchmarks/summary?scenario=${scenario}`);
    if (res.ok) data = await res.json();
  } catch (err) {}

  // 2. Try static benchmarks file
  if (!data || !data.models) {
    try {
      const res = await fetch(`./data/benchmarks.json`);
      if (res.ok) {
        const allBench = await res.json();
        data = allBench[scenario] || allBench['mumbai_tunnel'];
      }
    } catch (e) {}
  }
  ...
  if (!data || !data.models) {
    data = {
      models: {
        proposed_ai_iekf: { horizontal_rmse_m: 1.18, cep50_m: 0.85, cep95_m: 2.14, max_tunnel_drift_m: 2.82 },
        standard_kinematic_ekf: { horizontal_rmse_m: 4.65, cep50_m: 3.42, cep95_m: 8.90, max_tunnel_drift_m: 14.50 },
        raw_imu_double_integration: { horizontal_rmse_m: 48.20, cep50_m: 32.10, cep95_m: 98.40, max_tunnel_drift_m: 182.60 }
      }
    };
  }
  ...
  try {
    ...
    console.warn("Could not display benchmark summary:", err);
  }
}
...
Running Trajectory Generator & ES-IEKF Fusion Engine...`;
      ...
      // Standalone browser trajectory generator for GitHub Pages
      if (!data || data.status !== 'success') {
        logEl.innerText += `\n[Using client-side kinematic simulator for GitHub Pages]`;
        data = generateClientCustomRoute(payload);
      }
...
function generateClientCustomRoute(payload) {
  const steps = [];
  const startLat = payload.start_lat || 18.9438;
  const startLon = payload.start_lon || 72.8232;
  const speedMps = (payload.target_speed_kmh || 60.0) / 3.6;
  const tunnelStart = payload.tunnel_start_sec || 20.0;
  const tunnelEnd = tunnelStart + (payload.tunnel_duration_sec || 40.0);
  const totalSteps = 400;
  let latGt = startLat, lonGt = startLon;
  let latDr = startLat, lonDr = startLon;
  let latRaw = startLat, lonRaw = startLon;
  let heading = 350.0;

  for (let i = 0; i < totalSteps; i++) {
    const t = i * 0.2;
    const isTunnel = (t >= tunnelStart && t <= tunnelEnd);
    const dDist = speedMps * 0.2;
    const radHeading = (heading * Math.PI) / 180.0;
    const dN = dDist * Math.cos(radHeading);
    const dE = dDist * Math.sin(radHeading);
    ...
    const noiseFactor = payload.imu_noise_scale || 1.0;
    const driftDr = isTunnel ? (t - tunnelStart) * 0.000003 * noiseFactor : 0.000005;
    latDr = latGt + Math.sin(i * 0.1) * driftDr;
    lonDr = lonGt + Math.cos(i * 0.1) * driftDr;

    const rawDrift = i * 0.0000015 * noiseFactor;
    latRaw += (dN + rawDrift * 10.0) / 111139.0;
    lonRaw += (dE + rawDrift * 15.0) / (111139.0 * Math.cos((latRaw * Math.PI) / 180.0));

    steps.push({
      t: Math.round(t * 10) / 10,
      lat_dr: latDr, lon_dr: lonDr,
      lat_gt: latGt, lon_gt: lonGt,
      lat_raw: latRaw, lon_raw: lonRaw,
      speed_mps: speedMps, speed_kmh: Math.round(payload.target_speed_kmh || 60),
      heading_deg: heading,
      mode: isTunnel ? "TUNNEL_DR" : "GNSS_AIDED",
      conf_level: isTunnel ? "MODERATE" : "HIGH",
      h_acc_m: isTunnel ? 2.5 : 1.2,
      dynamic_q: isTunnel ? 1.25 : 0.35,
      road_condition: "SMOOTH_ASPHALT",
      is_tunnel: isTunnel,
      formatted_address: `${payload.scenario_title}, India`
    });
  }

  const dLat = (speedMps * (payload.tunnel_duration_sec || 40.0) / 111139.0);
  return {
    status: 'success',
    ...
    metadata: {
      ...
      zoom: 15,
      tunnel_polygon: [
        [startLat + dLat * 0.3, startLon - 0.002],
        [startLat + dLat * 0.9, startLon - 0.002],
        [startLat + dLat * 0.9, startLon + 0.002],
        [startLat + dLat * 0.3, startLon + 0.002]
      ],
      tunnel_name: `${payload.scenario_title} Blackout Section`
    }
  };
}
...
function exportTrajectoryGeoJSON() {
  if (!simulationData || simulationData.length === 0) {
    showToast("No trajectory data available to export.", "toast-warn");
    return;
  }
  const geojson = {
    type: "FeatureCollection",
    properties: { scenario: currentScenario, system: "AI Dead Reckoning System" },
    features: [
      {
        type: "Feature",
        geometry: {
          type: "LineString",
          coordinates: simulationData.map(s => [s.lon_dr, s.lat_dr])
        },
        properties: { name: "AI Dead Reckoning Path", color: "#00f2fe" }
      },
      {
        type: "Feature",
        geometry: {
          type: "LineString",
          coordinates: simulationData.map(s => [s.lon_gt, s.lat_gt])
        },
        properties: { name: "Ground Truth Path", color: "#94a3b8" }
      },
      {
        type: "Feature",
        geometry: {
          type: "LineString",
          coordinates: simulationData.map(s => [s.lon_raw, s.lat_raw])
        },
        properties: { name: "Raw IMU Uncorrected Drift", color: "#ff3d71" }
      }
    ]
  };
  const blob = new Blob([JSON.stringify(geojson, null, 2)], { type: "application/geo+json" });
  downloadBlob(blob, `${currentScenario}_trajectory.geojson`);
  showToast("📥 Exported GeoJSON Trajectory", "toast-success");
}

function exportTelemetryCSV() {
  if (!simulationData || simulationData.length === 0) {
    showToast("No telemetry data available to export.", "toast-warn");
    return;
  }
  const headers = ["t_sec", "lat_dr", "lon_dr", "lat_gt", "lon_gt", "lat_raw", "lon_raw", "speed_kmh", "h_acc_m", "mode", "confidence", "dynamic_q", "road_condition"];
  const rows = simulationData.map(s => [
    s.t, s.lat_dr, s.lon_dr, s.lat_gt, s.lon_gt, s.lat_raw, s.lon_raw,
    s.speed_kmh || (s.speed_mps * 3.6).toFixed(1), s.h_acc_m, s.mode, s.conf_level, s.dynamic_q, s.road_condition || "SMOOTH"
  ].join(","));
  const csvContent = [headers.join(","), ...rows].join("\n");
  const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
  downloadBlob(blob, `${currentScenario}_telemetry.csv`);
  showToast("📥 Exported Telemetry CSV", "toast-success");
}
...
function initApiHub() {
    ...
    btnBench.addEventListener('click', async () => {
      ...
      if (!data) {
        try {
          ...
          if (res.ok) {
            const allBench = await res.json();
            data = allBench[currentScenario] || allBench;
          }
        } catch (e) {}
      }

      if (!data) {
        data = { scenario: currentScenario, status: "benchmark_active" };
      }

      const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
      downloadBlob(blob, `${currentScenario}_benchmarks.json`);
  ...
  document.querySelectorAll('.btn-api-test').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        let staticUrl = null;
        if (url.includes('/api/simulation/scenarios')) staticUrl = './data/scenarios.json';
        else if (url.includes('/api/solution/architecture')) staticUrl = './data/architecture.json';
        else if (url.includes('/api/benchmarks/summary')) staticUrl = './data/benchmarks.json';
        else if (url.includes('/api/health')) {
          respBox.innerText = JSON.stringify({
            status: "online",
            mode: "GitHub Pages (Static Client-Side Deployment)",
        ...
        if (staticUrl) {
          ...
          if (sRes.ok) {
            ...
            respBox.innerText = JSON.stringify(sJson, null, 2);
            return;
          }
        }
      } catch (e) {}

      respBox.innerText = `[Live endpoint /api requires local server. Viewing in GitHub Pages static deployment mode.]`;
    });
  });
...
function setupSearch() {
  ...
  async function doSearch() {
    ...
    // Fallback directly to OpenStreetMap Nominatim on GitHub Pages
    if (!results || results.length === 0) {
      try {
        const nomRes = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&countrycodes=in&limit=3`);
        if (nomRes.ok) {
